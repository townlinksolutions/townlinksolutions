...
function customReplacer(match, p1, p2, p3, p4, p5, offset, string) {
    ...
    const baseSrc = p1.replace(/photo1\.(jpg|jpeg)$/, '');
    const ext = p1.match(/photo1\.(jpg|jpeg)$/) ? p1.match(/photo1\.(jpg|jpeg)$/)[1] : 'jpg';

    const eventId = "car_" + Math.random().toString(36).substr(2, 6);

    let imagesHtml = '';
    if (p1.includes('photo1')) {
        // Generate photo1 to photo6
        for(let i=1; i<=6; i++) {
           imagesHtml += `
                                    <img src="${baseSrc}photo${i}.${ext}" onerror="this.onerror=null; this.remove();" class="w-full flex-shrink-0 snap-center object-cover" alt="${p4}">`;
        }
    } else {
        // Just the original image + next ones by appending 2, 3...
        // Fallback
        imagesHtml = `
                                    <img src="${p1}" class="w-full flex-shrink-0 snap-center object-cover" alt="${p4}">`;
    }
...
// Regex looks for: <img src="(...)" class="w-full rounded-3xl shadow-2xl" alt="(...)">
