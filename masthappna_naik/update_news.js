...
const data = fs.readFileSync('src/news_data.js', 'utf8');
...
function updateNewsItem(id, shortDesc, content) {
    const regex = new RegExp(`(id:\\s*${id},[\\s\\S]*?shortDescEn:\\s*")[^"]*(",[\\s\\S]*?contentEn:\\s*")[^"]*(",)`, 'g');
    updatedData = updatedData.replace(regex, (match, p1, p2, p3) => {
        const escapedShortDesc = shortDesc.replace(/"/g, '\\"').replace(/\n/g, '\\n');
        const escapedContent = content.replace(/"/g, '\\"').replace(/\n/g, '\\n');
        return p1 + escapedShortDesc + p2 + escapedContent + p3;
    });
}

updateNewsItem(17, news17ShortDesc, news17Content);
updateNewsItem(21, news21ShortDesc, news21Content);
updateNewsItem(24, news24ShortDesc, news24Content);

fs.writeFileSync('src/news_data.js', updatedData, 'utf8');
console.log('Updated news_data.js');
